...
def process_commit_files(repo_full_name, commit_hash, commit_folder, session, processed_files_in_cve):
    """
    ...
    Args:
        ...
        processed_files_in_cve: Set de arquivos já processados no CVE (para evitar duplicação entre commits)
    ...
    try:
        ...
        # Criar arquivo de informações do commit
        ...
        with open(commit_info_path, 'w', encoding='utf-8') as f:
            f.write(f"=" * 70 + "\n")
            f.write(f"INFORMAÇÕES DO COMMIT\n")
            f.write(f"=" * 70 + "\n\n")
            f.write(f"Commit Hash: {commit_hash}\n")
            f.write(f"URL: {commit_data.get('html_url', 'N/A')}\n\n")
            
            author = commit_obj.get('author', {})
            f.write(f"Autor: {author.get('name', 'N/A')}\n")
            f.write(f"Email: {author.get('email', 'N/A')}\n")
            f.write(f"Data: {author.get('date', 'N/A')}\n\n")
            
            f.write(f"Mensagem do Commit:\n")
            f.write(f"{'-' * 70}\n")
            f.write(f"{commit_obj.get('message', 'N/A')}\n")
            f.write(f"{'-' * 70}\n\n")
            
            stats = commit_data.get('stats', {})
            f.write(f"Estatísticas Gerais:\n")
            f.write(f"  • Total de mudanças: {stats.get('total', 0)}\n")
            f.write(f"  • Linhas adicionadas: {stats.get('additions', 0)}\n")
            f.write(f"  • Linhas deletadas: {stats.get('deletions', 0)}\n")
            f.write(f"  • Arquivos modificados: {len(files)}\n\n")
            
            f.write(f"Arquivos Modificados:\n")
            f.write(f"{'-' * 70}\n")
            for idx, file_info in enumerate(files, 1):
                ...
                f.write(f"   Status: {status} | +{additions} -{deletions}\n")
        ...
        for file_info in files:
            ...
            # Evitar duplicação de arquivos no mesmo CVE
            if filename in processed_files_in_cve:
                print(f"         ⊘ {filename} (já processado em outro commit)")
                continue
            
            processed_files_in_cve.add(filename)
            ...
            if file_content:
                ...
                meta_path = file_full_path + '.metadata'
                with open(meta_path, 'w', encoding='utf-8') as f:
                    f.write(f"Arquivo: {filename}\n")
                    f.write(f"Commit: {commit_hash}\n")
                    f.write(f"Status: {status}\n")
                    f.write(f"Tamanho: {file_size} bytes\n")
                    f.write(f"Adições: +{file_info.get('additions', 0)} linhas\n")
                    f.write(f"Deleções: -{file_info.get('deletions', 0)} linhas\n")
                    f.write(f"Total de mudanças: {file_info.get('changes', 0)} linhas\n")
        ...
        for idx, row in df.iterrows():
            ...
            if pd.notna(repo) and repo != 'N/A':
                ...
                if commit_hashes:
                    ...
                    processed_files_in_cve = set()  # Para evitar duplicação de arquivos no mesmo CVE
                    ...
                    for c_idx, commit_hash in enumerate(commit_hashes, 1):
                        ...
                        files_count = process_commit_files(
                            repo, commit_hash, commit_folder, session, processed_files_in_cve
                        )
...
def create_readme_for_cve(row, cve_folder):
    ...
    try:
        ...
        with open(readme_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- `commit_XXXXXXXX/`: Pastas com arquivos de cada commit relacionado\n\n")
            f.write(f"### Conteúdo das pastas de commit:\n\n")
            f.write(f"- **Arquivos modificados**: Versão COMPLETA após o commit (sem sinais + e -)\n")
            f.write(f"- `_COMMIT_INFO.txt`: Informações detalhadas do commit\n")
            f.write(f"- `.metadata`: Metadados de cada arquivo (tamanho, mudanças, etc.)\n")
